...
#define o2_valve_pin  D7
#define air_servo_pin  D5
#define tube_servo_pin  D4
int relay_pin = D8;
...
void setup() {
  ...
  //servoIndex1 = STM32_ISR_Servos.setupServo(air_servo_pin, MIN_MICROS, MAX_MICROS);
  //servoIndex2 = STM32_ISR_Servos.setupServo(tube_servo_pin, MIN_MICROS, MAX_MICROS);
 ...
 */
  ...
  //STM32_ISR_Servos.setPosition(servoIndex1, int(air_servo_deg));
  ...
  //STM32_ISR_Servos.setPosition(servoIndex2, int(tube_servo_deg));
 ...
 // Serial.print(o2_set);
  ...
  SerialEvent2();
  o2_filtered = o2;
  pressure = (0.9 * pressure) + (0.1*pressure_message);
...
void SerialEvent2() {
  ...
  if (Serial2.available() > 0){
    ...
    myString += String(a);
    size_of_mystring = sizeof(myString);
   
    if (String(a) == "#"){
         
      if (13 >= myString.length() && myString.length() >= 12) {
      ...
      pressure_message = secondValue.toFloat();
      if (pressure_message > 80){
        pressure_message = 80;
      }
